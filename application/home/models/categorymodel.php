...
class Categorymodel extends CI_Model
{
	 ...
	 * @return void
	 */
	public function getCatePosition($cid,$area='',$pre='')
	{
		if ($cid==0)
		{
		...
		else
		{
			...
			$url = site_url(array('Article','index',$cid,$area));
			$this->position = " &gt; <a href='".$url."'>".$pre.$thiscate['c_name']."</a>".$this->position;
			self::getCatePosition($thiscate['p_id'],$area,$pre);			
		}
	...
	public function getReturn($cid)
	{
		...
		{
			//需要返回上一级按钮
			$returnurl = site_url(array('Article','index',$thiscate['p_id']));
			return $returnurl;
		}
		else
		{
