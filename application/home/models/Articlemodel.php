...
class Articlemodel extends CI_Model
{
	 ...
	 * @param $ser 位置分隔符
	 */
	public function getNPInfo($id,$area='',$pre='')
	{
        ...
        {	
        	if ($key==$prevk)
        	{
        		...
        		$npinfo['next']['title'] = $prevtitle = $pre.$prevarticle['a_fulltitle'];
        	}
        	if ($key==$nextk)
        	{
        		if ($k!=0)
        		{
        			...
        			$npinfo['prev']['title'] = $nexttitle = $pre.$nextarticle['a_fulltitle'];
        		}
        	}
        }
    	...
    	{
    		$npinfo['next']['url'] = $prevurl = site_url(array('Article','content',$prev,$area));
    	}
        if ($next=='')
        {
        ...
        {
        	$npinfo['prev']['url'] = $nexturl = site_url(array('Article','content',$next,$area));
        }
        return $npinfo;
	}
	...
	/*获取推荐文章10篇 按排序号->更新时间->文章ID降序排列*/
	public function getHot($cid,$cids,$area='')
	{
		$this->db->limit(5,0);
		$cids = $cid.$cids;
		$articlelist = $this->db->where_in('a_cid',explode(',', $cids))->order_by('a_hot desc,a_sort desc,updated_at desc,a_id desc')->get('article')->result_array();
		foreach ($articlelist as &$article)
		{
			$article['a_fulltitle'] = $area.$article['a_fulltitle'];
		}
		return $articlelist;
	}
