 ...
 * @return string
 */
function cut_str($str,$len,$charset="utf-8",$suffix="..."){
...
function html_to_data($data, $file) {
	$data = is_array($data) ? $data : array('cqzzcms_html_to_data' => $data);
...
function downfile($url) {
	return url('api/down', array('file' => str_replace('=', '', base64_encode(fn_authcode(array('cqzzcms' => $url), 'ENCODE')))));
}
...
function thumb($img, $width = null, $height = null) {
    ...
    if (empty($img) || strlen($img) == 1 ||  !@is_file($img)) {  
		return _thumb('/extensions/null.jpg', $width, $height); 
    }
	...
	if ($width && $height) {	//如果有宽高参数
		...
		if (empty($config['SITE_THUMB_TYPE']))	{	//静态模式，生成新图
			if (!file_exists($thumb)) {	//若文件不存在则生成新图
				$image = new image_lib();
				$image->set_image_size($width, $height)->make_limit_image2($img, $thumb,$width,$height);
			}
			return image($thumb);
		} else {
...
function _thumb($img, $width = null, $height = null) {
	return url('api/thumb', array('img' => str_replace('=', '', base64_encode(fn_authcode(array('cqzzcms' => $img), 'ENCODE'))), 'width' => $width, 'height' => $height));
}
function getKw($data) {
	...
	if (isset($data['setting']['url']['use']) && $data['setting']['url']['use'] == 1 && $data['setting']['url']['list']) {
		...
		if(!empty($city))
		{
			if (!strpos($url, 'index.php')) 
				if(strpos($url, '.html'))
					$url=str_replace('.html','_'.$city.'.html',$url);
				else
					$url=$city.'_'.$url; 	
		}
...
function showSeo($data, $page = 1) {
    ...
    return array('meta_title' => CITY_NAME.$meta_title, 'meta_keywords' => CITY_NAME.$meta_keywords, 'meta_description' => $meta_description);
}
...
function linkageform($linkageid = 0, $defaultvalue = 0, $id = 'linkage', $level = 3, $required) {
    ...
    for ($i = 1; $i <= $level; $i++) {
		...
		$string.='<select class="cqzzcms-select-' . $id . '" name="' . $id . '-' . $i . '" id="' . $id . '-' . $i .'" width="100" ' . $style . $required . '><option value=""> -- </option></select>&nbsp;&nbsp;';
	}
    $string.= '<script type="text/javascript">
				$(function(){
					var $ld5 = $(".cqzzcms-select-' . $id . '");					  
 ...
 * Tag标签URL
 */
function city_url($word) {
    global $config;
    if (empty($word)) return '';
    return $config['SITE_TAG_URL'] ? str_replace('{tag}', word2pinyin($word), SITE_PATH . $config['SITE_TAG_URL']) : url('city/list', array('kw' => word2pinyin($word)));
}
...
function fn_sendsms($mobile, $content) {
    ...
    $result = dr_catcher_data('http://sms.cqzz.net/index.php?uid='.$config['uid'].'&key='.$config['key'].'&mobile='.$mobile.'&content='.$content.'【'.$config['note'].'】&domain='.trim(str_replace('http://', '', SITE_URL), '/').'&sitename='.CMS_NAME);
    if (!$result) {
        return FALSE;
    }
...
function city($str) {
	...
	if(!empty($city))
	{
		...
		if (strpos($str, 'index.php'))
			$url=$str.'&city='.$city;				
		else 
			if(strpos($str, '.html'))
				$url=str_replace('.html','_'.$city.'.html',$str);
			else
				$url='/'.$city.'_'.substr($str,1); 	
	}
	else
		$url=$str;	
	return $url;
 ...
 * 拆分后分别加上城市 
 */
function add_city($title,$str)
{
	$_title = explode($str, $title);
	foreach ($_title as $t) {
		$city_title.=empty($city_title)? CITY_NAME . $t:$str.CITY_NAME . $t;
	}
	return $city_title;
}
...
//获取服务端授权文件校验
/*
$myfile = fopen("authority.txt", "r") or $myfile = fopen("authority.txt", "w");
$domain=fread($myfile,filesize("authority.txt"));
fclose($myfile);


$servername=get_yuming();
if($domain){ 
	j($domain,false); 
}else{
	$domain = file_get_contents('http://phpmb.cqzz.net/index.php?c=Copyright&a=search&domain='.$servername); 
	j($domain,true); 
}
function j($strc,$iswrite)
{	
	$servername=get_yuming();
	$key = 'mycqzz'; 
	$str= authcode($strc,'DECODE',$key,0); //解密	
	$arr = explode('|',$str); 
 
	if($arr[0]==$servername&&$arr[1]>time()){   
	}else{
		$myfile = fopen("authority.txt", "w") or die("Unable to open file!");		 
		fwrite($myfile, '');
		fclose($myfile);
		echo '<div style="text-align:center"><img src="http://phpmb.cqzz.net/views/cqzz/images/guoqi.jpg"></div>';
		exit;
	} 	
	if($iswrite){
		$myfile = fopen("authority.txt", "w") or die("Unable to open file!");		 
...
function get_yuming()
{
	...
	$ymarry = explode('.', $servername);
	$servername = $ymarry[count($ymarry) - 2] . '.' . $ymarry[count($ymarry) - 1];
	return $servername;
}
